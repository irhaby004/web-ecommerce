...
// Open single product
Route::get('{any}-{id}', 'Publics\ProductsController@productPreview')
    ->where('id', '[\d+]+')->where('any', '(.*)');
...
/* Cart Routes for AJAX */
Route::prefix('cart')->group(function () {
    Route::post('add', 'Publics\CartController@addProduct')->name('cart.add');
    Route::post('render', 'Publics\CartController@renderCartProductsWithHtml')->name('cart.render');
    Route::post('remove-quantity', 'Publics\CartController@removeProductQuantity')->name('cart.remove_quantity');
    Route::post('checkout-products', 'Publics\CartController@getProductsForCheckoutPage')->name('cart.checkout_products');
    Route::post('remove', 'Publics\CartController@removeProduct')->name('cart.remove');
});
...
Route::middleware(['auth'])->group(function () {
    ...
    Route::get('admin/orders/export-apriori', 'Admin\OrdersController@exportApriori')
        ->name('admin.orders.export_apriori');

    // Jika pakai locale
    Route::get('{locale}/admin/orders/export-apriori', 'Admin\OrdersController@exportApriori')
        ->where('locale', implode('|', Config::get('app.locales')));
    /* APRIORI ROUTES */
    // Menampilkan halaman upload
    Route::get('admin/apriori', 'Admin\AprioriUploadController@index')
        ->name('admin.apriori.index');

    // Menampilkan halaman upload dengan locale (opsional)
    Route::get('{locale}/admin/apriori', 'Admin\AprioriUploadController@index')
        ->where('locale', implode('|', Config::get('app.locales')));

    // Menangani upload dataset
    Route::post('admin/apriori/upload', 'Admin\AprioriUploadController@store')
        ->name('admin.apriori.upload.store');

    // Menangani upload dataset dengan locale (opsional)
    Route::post('{locale}/admin/apriori/upload', 'Admin\AprioriUploadController@store')
        ->where('locale', implode('|', Config::get('app.locales')));
});
...
/* Registration Routes. We don't need it
Route::get('register', [
    'as' => 'register',
